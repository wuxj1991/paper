...
tt$DebtRatio<-NULL
tt<-tt[,c(1:5,12,6:11)]
tt.pca<-princomp(tt[,c(-1,-2)],cor=TRUE)
biplot(tt.pca)
summary(tt.pca,loadings=TRUE)
screeplot(tt.pca,type="lines")
#logistic
reg<-glm(SeriousDlqin2yrs~.,family=binomial(link='logit'),data=tt[,c(-1)])
reg1<-step(reg,direction=c("backward"))
reg_pred<-predict(step(reg1,direction=c("backward")),type ="response")
...
log.test_pred<-predict(step(reg1,direction=c("backward")),data=test1)
